...
function isAnimationDisabled(props) {
...
function findChildByKey(key, state) {
  return find(function (child) {
    return getKey(child) === key;
  }, state.children);
}

function calculateNextSetOfChildren(nextChildren, nextProps, previousState) {
  // We want to:
  ...
  var updatedChildren = nextChildren.map(function (nextChild) {
    var child = findChildByKey(nextChild.key, previousState);
  ...
  previousState.children.forEach(function (child, index) {
...
var FlipMove$1 = function (_Component) {
  ...
  FlipMove.getDerivedStateFromProps = function getDerivedStateFromProps(nextProps, previousState) {
    ...
    return isAnimationDisabled(nextProps) ? nextChildren.map(function (element) {
      return _extends({}, element, {
        element: element
      });
  ...
  FlipMove.prototype.componentDidMount = function componentDidMount() {
    ...
    // component mounts.
    var shouldTriggerFLIP = this.props.appearAnimation && !isAnimationDisabled(this.props);
  ...
  FlipMove.prototype.componentDidUpdate = function componentDidUpdate(previousProps, previousState) {
    // When the component is handed new props, we need to figure out the
    // "resting" position of all currently-rendered DOM nodes.
    // We store that data in this.parent and this.children,
    // so it can be used later to work out the animation.
    ...
    var shouldTriggerFLIP = !arraysEqual(oldChildrenKeys, nextChildrenKeys) && !isAnimationDisabled(this.props);
  ...
  FlipMove.prototype.prepForAnimation = function prepForAnimation() {
    ...
    if (leaveAnimation) {
      ...
      leavingChildren.forEach(function (leavingChild) {
        var childData = _this2.getChildData(getKey(leavingChild));

        // Warn if child is disabled
        if (!isAnimationDisabled(_this2.props) && childData.domNode && childData.domNode.disabled) {
          childIsDisabled();
        }
        ...
        if (childData.boundingBox) {
          removeNodeFromDOMFlow(childData, _this2.props.verticalAlignment);
        }
      });
    ...
    this.state.children.forEach(function (child) {
      var _getChildData = _this2.getChildData(getKey(child)),
  ...
  FlipMove.prototype.animateChild = function animateChild(child, index, childInitialStyles) {
    ...
    requestAnimationFrame(function () {
      requestAnimationFrame(function () {
        ...
        var styles = {
          transition: createTransitionString(index, _this3.props),
          transform: '',
          opacity: ''
        };

        if (child.appearing && _this3.props.appearAnimation) {
          styles = _extends({}, styles, _this3.props.appearAnimation.to);
        } else if (child.entering && _this3.props.enterAnimation) {
          styles = _extends({}, styles, _this3.props.enterAnimation.to);
        } else if (child.leaving && _this3.props.leaveAnimation) {
          styles = _extends({}, styles, _this3.props.leaveAnimation.to);
        }
  ...
  FlipMove.prototype.bindTransitionEndHandler = function bindTransitionEndHandler(child) {
    ...
    var transitionEndHandler = function transitionEndHandler(ev) {
      ...
      if (child.leaving) {
        _this4.removeChildData(getKey(child));
      }
    };
    ...
    if (this.remainingAnimations === 0) {
      ...
      this.setState({ children: nextChildren }, function () {
        if (typeof _this5.props.onFinishAll === 'function') {
          _this5.callChildrenHook(_this5.props.onFinishAll);
        }

        // Reset our variables for the next iteration
        _this5.childrenToAnimate = [];
      });
  ...
  FlipMove.prototype.callChildrenHook = function callChildrenHook(hook) {
    ...
    this.childrenToAnimate.forEach(function (childKey) {
      ...
      // If so, skip it.
      var child = findChildByKey(childKey, _this6.state);
      ...
      if (_this6.hasChildData(childKey)) {
        domNodes.push(_this6.getChildData(childKey).domNode);
      }
    });
  ...
  FlipMove.prototype.updateBoundingBoxCaches = function updateBoundingBoxCaches(previousProps, previousState) {
    var _this7 = this;
    ...
    previousState.children.forEach(function (child) {
      ...
      var childData = _this7.getChildData(childKey);
  ...
  FlipMove.prototype.createHeightPlaceholder = function createHeightPlaceholder() {
    ...
    return React.createElement(placeholderType, {
      ...
      ref: function ref(domNode) {
        _this8.heightPlaceholderData.domNode = domNode;
      },
    ...
    return this.state.children.map(function (child) {
      return React.cloneElement(child.element, {
        ref: function ref(element) {
          ...
          _this9.setChildData(getKey(child), { domNode: domNode });
        }
      });
    });
  };
  ...
  FlipMove.prototype.render = function render() {
    ...
    var props = _extends({}, delegated, {
      ...
      ref: function ref(node) {
        _this10.parentData.domNode = node;
      }
    });
